...
const SearchLocation = (props: SearchLocationProps) => {
  ...
  const placeServices = useRef<kakao.maps.services.Places | null>(null);

  useEffect(() => {
    if (placeServices.current) {
      return;
    }

    placeServices.current = new kakao.maps.services.Places();
  }, []);
  ...
  const searchPlaces = (keyword: string) => {
    ...
    if (!placeServices.current) {
      // TODO: placeService 에러 핸들링
      alert("에러");
      return;
    }

    placeServices.current.keywordSearch(keyword, (data, status) => {
